...
function createWorkerFactory(script) {
  ...
  function createWorker({
    ...
    if (scriptUrl) {
      const endpoint = rpc.createEndpoint(createMessenger(scriptUrl), endpointOptions);
      const {
        call: caller
      } = endpoint;
...
function expose(caller, api) {
  ...
  endpoint === null || endpoint === void 0 || endpoint.expose(api);
  return endpoint != null;
}
function terminate(caller) {
  const endpoint = getEndpoint(caller);
  endpoint === null || endpoint === void 0 || endpoint.terminate();
